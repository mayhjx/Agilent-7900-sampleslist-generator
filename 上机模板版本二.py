...
def Save(Patch):
    ...
    if App.Std.get() == 1:
        ...
        for i in range(0,len(Lists)):
            ...
            ws.cell(row=(i+14),column=5,value=20)
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        # 标准曲线
        self.Std = tk.IntVar()
        self.Check = tk.Checkbutton(self, text="标准曲线",variable=self.Std)
        self.Check.deselect()
        self.Check.place(relx=.3,rely=.7,anchor=tk.CENTER)

        self.Creat_Button = ttk.Button(self,text="生成",command=Main)
        self.Creat_Button.place(relx=.3,rely=.86,anchor=tk.CENTER)

        self.Quit_Button = ttk.Button(self,text="退出",command=self.destroy)
